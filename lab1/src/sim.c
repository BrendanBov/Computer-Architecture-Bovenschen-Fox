...
int i_process(char* i_) {
    ...
    else if (!strcmp(d_opcode, "1100111")) //opcode 103
    {
        if (Funct3 == 0)
        {
...
int j_process(char* i_) {
    ...
    imm[20] = '0';
    for (int i = 0; i < 10; i++) {
        imm[i + 10] = i_[31 - 30 + i]; //imm 10 to 19,i_ 1 to 10
    }
    imm[9] = i_[31 - 20]; //imm 9, i_ 11
    for (int i = 0; i < 8; i++) {
        imm[i + 1] = i_[31 - 19 + i]; //imm 1 to 8,i_ 12 to 19
    }
    imm[0] = i_[31 - 31]; //imm 0, i_ 0
    ...
    if (!strcmp(d_opcode, "1101111")) { //opcode 111
...
int u_process(char* i_) {
    ...
    if (!strcmp(d_opcode, "0010111")) { //opcode 23
        printf("--- This is an AUIPC instruction. \n");
        AUIPC(Rd, UpImm);
        return 0;
    }
    else if (!strcmp(d_opcode, "0110111")) { //opcode 55
